...
func (g *Graph) addNode(cid string, label string, annotations []string) Node {
...
func (n Node) dfs() []string {
	...
	explore = func(t Node) {
		for label, node := range t.Connections {
			...
			if !explored && label != PublicInternetLabel {
				explore(node)
			}
		}
	}
...
func (n Node) dfsWithACL() []string {
	...
	explore = func(t Node) {
		for label, node := range t.Connections {
			if _, ok := node.Annotations[aclAnnotation]; ok {
				reached[label] = struct{}{}
			}

			if _, explored := reached[label]; !explored {
				reached[label] = struct{}{}
				explore(node)
			}
		}
