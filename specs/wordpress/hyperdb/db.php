...
class hyperdb extends wpdb {
	...
	function db_connect( $query = '' ) {
		...
		if ( empty( $this->hyper_servers ) ) {
			...
			$this->dbh = @ $connect_function("{$host_prefix}{$DB_HOST}", DB_USER, DB_PASSWORD);
			if ( !  $this->dbh  )
		...
		while ( isset( $this->dbhs[$dbhname] ) &&  $this->dbhs[$dbhname]  ) {
		...
		do {
			...
			foreach ( $servers as $group_key ) {
				...
				{
					$this->dbhs[$dbhname] = @ $connect_function( "{$host_prefix}$host:$port", $user, $password );	
				} else {
			...
			if ( !$success || !isset($this->dbhs[$dbhname]) || ! $this->dbhs[$dbhname]  ) {
	...
	function check_database_version( $dbh_or_table = false ) {
		...
		$mysqli_version = preg_replace( '|[^0-9\.]|', '', $this->db_version( $dbh_or_table ) );
